...
export function LendersScreen() {
  ...
  const queryResult = useQuery<Lender[]>({ queryKey: ['lenders'], queryFn: fetchLenders }) as any;
  const lenders: Lender[] = queryResult.data ?? [];
  ...
  const createMutation = useMutation({
    mutationFn: () => createLender({ name: name.trim(), code: code.trim().toUpperCase() }),
    onSuccess: () => {
      invalidateQueries(['lenders']);
      setShowModal(false);
      setName('');
      setCode('');
      Alert.alert('Lender added', 'The lender has been added successfully.');
    },
    onError: (error: any) => {
      Alert.alert('Error', error?.response?.data?.message ?? 'Could not create lender.');
    },
  });

  const handleCreate = () => {
    if (!name.trim()) return Alert.alert('Required', 'Enter the lender name.');
    if (!code.trim()) return Alert.alert('Required', 'Enter the lender code (e.g. HDFC, SBI).');
  ...
  if (isError) {
    return (
      <View style={styles.centered}>
        ...
        <Text style={styles.errorText}>Could not load lenders</Text>
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        <Text style={styles.headerTitle}>Lenders</Text>
        <Text style={styles.headerSub}>{lenders.length} lender{lenders.length !== 1 ? 's' : ''}</Text>
      </View>
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View style={styles.emptyBox}>
            <MaterialIcons name="account-balance" size={40} color="#C8DFD0" />
          ...
          <View style={styles.card}>
            <View style={styles.cardLeft}>
              <View style={styles.iconBox}>
                <MaterialIcons name="account-balance" size={20} color={BRAND.primary} />
              </View>
              <View style={{ flex: 1 }}>
            ...
            <View style={[styles.statusBadge, { backgroundColor: l.isActive ? '#27AE6020' : '#E67E2220' }]}>
              <Text style={[styles.statusText, { color: l.isActive ? '#27AE60' : '#E67E22' }]}>
        ...
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={{ flex: 1 }}>
          <ScrollView contentContainerStyle={styles.modalContent} keyboardShouldPersistTaps="handled">
            <View style={styles.modalHeader}>
              <Text style={styles.modalTitle}>Add Lender</Text>
            ...
            <View style={{ marginTop: 12 }}>
              <PrimaryButton
                label={createMutation.isPending ? 'Adding...' : 'Add Lender'}
