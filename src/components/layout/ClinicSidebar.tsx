...
export function ClinicSidebar({ mobileOpen = false, onClose }: SidebarProps) {
  ...
  const navItems = [
    { href: '/dashboard/clinic', label: 'Dashboard', icon: IconDashboard },
    { href: '/dashboard/clinic/leads', label: 'My Leads', icon: IconLeads },
    { href: '/dashboard/clinic/disbursals', label: 'Disbursals', icon: IconMoney },
  ]
  ...
  return (
    <aside className={asideClass}>
      ...
      <div className="flex-shrink-0 px-3 py-3 border-t border-white/10">
        ...
        <Link
          href="/dashboard/clinic/change-password"
          onClick={onClose}
          className={cn(
            'w-full text-left px-2 py-1.5 text-xs rounded-md transition flex items-center gap-2',
            pathname === '/dashboard/clinic/change-password'
              ? 'bg-trustiva-lime text-trustiva-navy font-semibold'
              : 'text-slate-300 hover:bg-white/10 hover:text-white'
          )}
        >
