...
// ── Designation system — imported from @/lib/hr/designations ──────────────────

// Friendly system-role labels
                    ...
                    <tr key={u.id} className="hover:bg-gray-50 transition">
                      ...
                      <td className="px-4 py-3">
                        <div className="flex gap-2 flex-wrap">
                          ...
                          {isSuperAdmin && (
                            <button
                              onClick={() => { setRoleChangeUser(u); setNewRole('') }}
...
function UserForm({
  ...
  // Filter Reporting To options based on designation hierarchy
  const reportingOptions = useMemo(() => {
    ...
    return allUsers.filter(u => {
      if (!u.designation) return true // users with no designation are always shown
      const theirLevel = DESIGNATION_LEVEL[u.designation]
      if (!theirLevel) return true
      return theirLevel < myLevel // show only higher-level users
    })
      ...
      {tab === 'info' && (
        <div className="space-y-4">
          ...
          <div>
            ...
            <input
              type="text" value={form.name} onChange={e => upd('name', e.target.value)} required
              placeholder="e.g. Ajit Yadav"
          ...
          {!isEdit && (
            <div>
              ...
              <input
                type="email" value={form.email} onChange={e => upd('email', e.target.value)} required
          ...
          <div>
            ...
            <input
              type="password" value={form.password} onChange={e => upd('password', e.target.value)}
          ...
          <div className="border-t border-gray-100 pt-4 space-y-4">
            ...
            <div>
              ...
              <select
                ...
                onChange={e => { upd('designation', e.target.value); upd('reportingManagerId', '') }}
                className="w-full px-3 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-brand-400"
              >
              ...
              {form.designation && DESIGNATION_LEVEL[form.designation] && (
                <p className="text-xs text-brand-600 mt-1 font-medium">
                  Hierarchy level {DESIGNATION_LEVEL[form.designation]} of 6
                </p>
              )}
            </div>
              ...
              <div className="grid grid-cols-2 gap-2">
                {[
                  ...
                  <button
                    key={opt.value}
                    type="button"
                    onClick={() => upd('role', opt.value)}
                    className={cn(
                      'text-left px-3 py-2.5 rounded-xl border-2 transition text-sm',
                      form.role === opt.value
              ...
              <label className="block text-xs font-semibold text-gray-600 mb-1.5">
                ...
                {form.designation && reportingOptions.length < allUsers.length && (
                  <span className="ml-2 font-normal text-amber-600 text-xs">
                    Filtered to {reportingOptions.length} user{reportingOptions.length !== 1 ? 's' : ''} above your level
                  </span>
                )}
              </label>
              <select
                value={form.reportingManagerId}
                onChange={e => upd('reportingManagerId', e.target.value)}
                className="w-full px-3 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-brand-400"
              >
                <option value="">— No reporting manager —</option>
                {reportingOptions.map(u => (
                  <option key={u.id} value={u.id}>{formatManagerLabel(u)}</option>
                ))}
              </select>
              {reportingOptions.length === 0 && form.designation && (
                <p className="text-xs text-gray-400 mt-1">
                  No users found at a higher level. Add senior users first.
                </p>
              )}
            </div>
          </div>
          ...
          {(form.role === 'ADMIN' || form.role === 'SUPER_ADMIN') && (
            <div className="bg-purple-50 border border-purple-200 rounded-xl p-4">
              <p className="text-sm font-semibold text-purple-800">
                {form.role === 'SUPER_ADMIN' ? 'Super Admin' : 'Admin'} — Full System Access
              </p>
              <p className="text-xs text-purple-600 mt-1">
                No region or clinic assignment needed. This role has unrestricted access.
              </p>
            </div>
          )}
          ...
          {form.role === 'REGIONAL_MANAGER' && (
            <div className="space-y-3">
              <div className="bg-teal-50 border border-teal-200 rounded-xl p-3">
                ...
                <p className="text-xs text-teal-600 mt-0.5">Manager can view data for assigned regions only</p>
              </div>
              <div className="border border-gray-200 rounded-xl p-3 max-h-48 overflow-y-auto space-y-1.5">
                {regions.length === 0 && (
                  <p className="text-xs text-gray-400">No regions found — create a region first</p>
                )}
                {regions.map(r => (
                  <label key={r.id} className="flex items-center gap-2 cursor-pointer hover:bg-gray-50 px-2 py-1 rounded-lg">
                    <input
                      ...
                      onChange={e => setSelectedRegions(prev =>
              ...
              {selectedRegions.length > 0 && (
                <p className="text-xs text-teal-600 font-medium">✓ {selectedRegions.length} region(s) assigned</p>
              )}
            </div>
          )}
          ...
          {form.role === 'TEAM_MEMBER' && (
            <div className="space-y-3">
              <div className="bg-green-50 border border-green-200 rounded-xl p-3">
                ...
                <p className="text-xs text-green-600 mt-0.5">Team Member can view assigned clinic data and add leads</p>
              </div>
              <div className="border border-gray-200 rounded-xl p-3 max-h-48 overflow-y-auto space-y-1.5">
                {clinics.length === 0 && (
                  <p className="text-xs text-gray-400">No clinics found — onboard a clinic first</p>
                )}
                {clinics.map(c => (
                  <label key={c.id} className="flex items-center gap-2 cursor-pointer hover:bg-gray-50 px-2 py-1 rounded-lg">
                    <input
                      ...
                      onChange={e => setSelectedClinics(prev =>
              ...
              {selectedClinics.length > 0 && (
                <p className="text-xs text-green-600 font-medium">✓ {selectedClinics.length} clinic(s) assigned</p>
              )}
            </div>
          )}
        </div>
