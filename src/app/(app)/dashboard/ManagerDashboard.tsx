...
interface ManagerData {
  regionName: string;
  kpi: { totalLeads: number; approvedLeads: number; disbursedLeads: number; pendingLeads: number; rejectedLeads: number; approvedNotDisbursed: number; approvalRate: number; totalClinics: number; currentMonthLeads: number; lastMonthLeads: number; mtdDisbursalValue: number; lmtdDisbursalValue: number; };
  chartData: { month: string; leads: number; approved: number; disbursed: number }[];
  rmWise: { name: string; leads: number; approved: number; disbursed: number; mtd: number; approvalRate: number }[];
  lenderWise: { name: string; leads: number; approved: number; disbursed: number; approvalRate: number }[];
  clinicWise: { name: string; code: string; rm: string; total: number; approved: number; disbursed: number; mtd: number; lmtd: number; growth: number }[];
}

type TabKey = "overview" | "rm" | "clinic" | "lender";
...
export default function ManagerDashboard() {
  ...
  const downloadExcel = async () => {
    ...
    a.href = url;
    a.download = `region-leads-${new Date().toISOString().slice(0,10)}.csv`;
    a.click();
  };
    ...
    <div className="p-4 space-y-5 max-w-7xl mx-auto">
      ...
      {activeTab === "overview" && (
        <div className="bg-white rounded-xl shadow p-4">
          <h2 className="text-base font-semibold mb-3">6-Month Lead Trend — {data.regionName}</h2>
            ...
            <>
              <ResponsiveContainer width="100%" height={240}>
                <BarChart data={data!.rmWise}>
                  ...
                  <Bar dataKey="disbursed" fill="#a855f7" radius={[4,4,0,0]} name="Disbursed" />
                </BarChart>
              </ResponsiveContainer>
              <Table headers={["RM Name","Total","Approved","Disbursed","MTD","Approval Rate"]}
                rows={data!.rmWise.map((r) => [r.name, r.leads, r.approved, r.disbursed, r.mtd, `${r.approvalRate}%`])} />
            </>
          )}
        </div>
      )}
      ...
      {activeTab === "lender" && (
        <div className="bg-white rounded-xl shadow p-4">
          <h2 className="text-base font-semibold mb-3">Lender Wise Performance</h2>
...
function KPICard({ label, value, color, suffix = "", badge, badgeColor }: { label: string; value: number; color: string; suffix?: string; badge?: string; badgeColor?: string; }) {
  const colors: Record<string, string> = { blue: "bg-blue-50 text-blue-700 border-blue-200", green: "bg-green-50 text-green-700 border-green-200", purple: "bg-purple-50 text-purple-700 border-purple-200", yellow: "bg-yellow-50 text-yellow-700 border-yellow-200", orange: "bg-orange-50 text-orange-700 border-orange-200", teal: "bg-teal-50 text-teal-700 border-teal-200", indigo: "bg-indigo-50 text-indigo-700 border-indigo-200" };
  return (
